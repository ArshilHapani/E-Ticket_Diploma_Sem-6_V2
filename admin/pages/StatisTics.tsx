...
const StatisTics = () => {
  ...
  const [ticketsPaymentsCount, setTicketsPaymentsCount] =
    useState<Array<number>>();
  const [ticketCount, setTicketCount] = useState<object>({
    dates: [],
    counts: [],
  });
  useEffect(() => {
  ...
  const data1 = {
    ...
    datasets: [
      {
        label: "number of tickets generated",
  ...
  const data = {
    ...
    datasets: [
      {
        label: "Recharges added by conductors",
  ...
  const userData = {
    labels: ["users", "conductors", "admins"],
    datasets: [
      {
        label: "User counts",
  ...
  return (
    <>
      <Container className="mt-[13vh]">
        <Typography variant="h4" textAlign="center" sx={{ margin: "1rem 0" }}>
          Analysis
        </Typography>
