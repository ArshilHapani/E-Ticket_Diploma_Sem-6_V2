    ...
    try {
        ...
        con.query(fetchID, (err, qres) => {
            if (err) {
                ...
                const fetchPayment = `SELECT payment.pay_time, payment.pay_amount, payment.pay_id, conductor.c_uname FROM payment, conductor WHERE payment.p_id='${qres[0].p_id}' && payment.c_id = conductor.c_id ORDER BY pay_time DESC;`;
